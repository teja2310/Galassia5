...
class ObjectDetectionUtils:
    ...
    def draw_detection(self, draw: ImageDraw.Draw, box: list, cls: int, score: float, color: tuple, scale_factor: float):
        ...
        draw.rectangle([(xmin*0.80, ymin*0.80), (xmax*0.80, ymax*0.80)], outline=color, width=2)
        draw.text((xmin*0.80 + 4, ymin*0.80 + 4), label, fill=color, font=font)

    def visualize(self, detections: dict, image: Image.Image, image_id: int, output_path: str, width: int, height: int, min_score: float = 0.0, scale_factor: float = 1.0):
        """
        ...
        for idx in range(detections['num_detections']):
            if scores[idx] >= min_score:
                ...
                scaled_box = [x * height if i % 2 == 0 else x * width for i, x in enumerate(boxes[idx])]
                ymin, xmin, ymax, xmax= scaled_box
                dets.append([xmin*0.80, ymin*0.80, xmax*0.80, ymax*0.80, scores[idx]])
    ...
    def extract_detections(self, input_data: list, threshold: float = 0.0) -> dict:
        """
